...
const Navbar = () => {
    ...
    const [navDrawerOpen, setNavDrawerOpen] = useState(false);  // Set default to false for initial state
    const { cart } = useSelector((state) => state.cart);
    const {user}=useSelector((state)=>state.auth)

    const cartItemCount = cart?.products?.reduce((total, product) => total + product.quantity, 0) || 0;

    const toggleNavDrawer = () => setNavDrawerOpen(prevState => !prevState);
    const cartDrawer = () => setDrawerOpen(prevState => !prevState);
        ...
        <>
            <nav className="container mx-auto flex items-center justify-between px-6 py-4">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <button onClick={cartDrawer} className="relative hover:text-black" aria-label="Open Cart Drawer">
            ...
            <CartDrawer drawerOpen={drawerOpen} cartDrawer={cartDrawer} />

            {/* Mobile Menu Drawer */}
            <div
                className={`md:hidden fixed top-0 left-0 w-3/4 sm:w-1/2 md:1/3 h-full bg-white shadow-lg transform transition-all duration-300 ease-in-out z-50 
