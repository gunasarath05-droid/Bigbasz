...
const ProductDetails = ({ productId }) => {
  ...
  // Use either passed productId prop or id from URL
  const productFetchId = productId || id;

  useEffect(() => {
    if (productFetchId) {
      dispatch(fetchProductDetails(productFetchId));
      dispatch(fetchSimilarProduct(productFetchId )); // Pass only ID, not an object
    }
                ...
                <div className="flex gap-2 mt-2">
                  {selectedProduct.sizes.map((size) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded border border-gray-300 ${selectedSize === size ? 'bg-black text-white' : ''
                        }`}
                    >
                      {size}
                    </button>
